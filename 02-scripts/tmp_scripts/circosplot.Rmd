...
```{r}
#primary_tumor_patient_stratification_tumor_cells <- #read.csv("/data/projects/2022/CRCA/results/v1/final/liana_cell2cell/core_atlas/tumor_normal/epithelial_cancer/epithelial#_cancer.csv", row.names=1)
primary_tumor_patient_stratification_tumor_cells <- read.csv("/data/scratch/kvalem/projects/2022/crc_atlas_circos_plot/02-scripts/final_cpdb.csv", row.names=1)
```


```{r}
library(data.table)
primary_tumor_patient_stratification_tumor_cells <- fread("/data/scratch/kvalem/projects/2022/crc_atlas_circos_plot/02-scripts/final_cpdb.csv")
```
...
```{r}
immune_cells_blood<- c('B cell','CD4', 'CD8','Cancer cell ciruclating',"Dendritic cell","Eosinophil",'Macrophage','Monocyte', "Platelet",'Treg',"TAN4")
```

```{r}
immune_cells_blood <- list(unique(primary_tumor_patient_stratification_tumor_cells$source))
```

```{r}
immune_cells = list(unique(primary_tumor_patient_stratification_tumor_cells$cell_type))
```

```{r}
immune_cells = c("Cancer cell",  "Macrophage", "Endothelial","Monocyte","Neutrophil", "NK","T cell CD4" ,"T cell regulatory", "B cell","T cell CD8","cDC","pDC")   
```
...
ggsave("circos_plot_core_atlas_new_last.pdf", plot = p, width = 55, height = 45, units = "cm")
```
