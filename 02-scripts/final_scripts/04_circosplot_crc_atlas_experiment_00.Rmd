...
tumor_normal_interactions = liana_res|>
  select(source=source_genesymbol, target=target_genesymbol, interaction_stat, fdr=interaction_padj, log2_fc=interaction_expr, cluster_2=source, interaction_pvalue) |>
...
```{r}
top_genes = tumor_normal_interactions |> select(source, fdr) |> distinct() |> arrange(fdr) |> pull(source) |> head(20)
```
...
```{r}
cpdb_res <- cpdb_res[!cpdb_res$cluster_2 %in% c("Tuft", "Schwann cell", "gamma-delta","Enteroendocrine","NK","Pericyte","Plasma cell"), ]
...
select(from, to, name="variable", coef, interaction_pvalue, fdr, log2_fc) |>
...
nodes_target = cpdb_res |>
  select(cell_type=cluster_2, gene=target, interaction_stat) |>
  mutate(type="receptor") |>
  mutate(name=sprintf("%s_%s_%s", cell_type, gene, type))
nodes = bind_rows(nodes_source, nodes_target) |>
  distinct() |>
  select(name, cell_type, gene, type, interaction_stat) |>
...
nodes_ct_hierarchy = data.frame(name=unique(c(edges_ct_hierarchy$to, edges_ct_hierarchy$from))) |>
  left_join(nodes, by="name") |> group_by(cell_type) |>  group_modify(function(x, key) {
    ...
    x
  }) |> ungroup() |> select(name, cell_type, gene, type, cell_type_label, interaction_stat)

nodes_ct_hierarchy <- nodes_ct_hierarchy[!duplicated(nodes_ct_hierarchy$name), ]
...
p = ggraph(G_ct, layout="dendrogram", circular=TRUE) +
  geom_node_point(aes(color=cell_type, shape=type, size=interaction_stat), stroke=2, fill="white") +
  geom_conn_bundle(
    data = get_con(from=from, to=to, coef=conn_meta$coef, log2_fc=conn_meta$log2_fc, pvalue=conn_meta$pvalue,  fdr=conn_meta$fdr),
...
ggsave(paste0(path,"circos_plot_CD4.pdf"), plot = p, width = 55, height = 45, units = "cm")
```
