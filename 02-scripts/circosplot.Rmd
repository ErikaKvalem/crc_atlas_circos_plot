...
```{r}
#edge_color_scale=scale_edge_color_gradientn(colors=pals::coolwarm(100), limits=c(-0.9,0.9) * #max(abs(conn_meta[[edge_color]])))
```

```{r}
edge_color_scale <- scale_edge_color_gradientn(
  colors = pals::coolwarm(100),
 limits = c(0, 12),   values = scales::rescale(c(0, 5, 12), to = c(0, 1)) 
)
...
p = ggraph(G_ct, layout="dendrogram", circular=TRUE) +
 ...
 theme(legend.position="bottom",legend.margin = margin(t = 1.5, unit = "cm"),legend.key.size = unit(0.8, "cm"), plot.margin = margin(2, 3, 3, 3, "cm"),
  ...
  ggtitle("Crosstalk of Neutrophil with major immune cells in tumor vs blood. Top 10 DEG")
